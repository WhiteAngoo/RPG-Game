            ...
            >
              <div className="relative flex items-center justify-center">
                ...
                {isSelected ? (
                  ...
                  <div className={`w-3 h-3 rounded-full border-2 shadow-[0_0_5px_rgba(0,0,0,0.8)] transition-colors ${isCurrent ? 'bg-amber-500 border-white' : 'bg-slate-800 border-slate-500 hover:bg-slate-600'
                    }`}
                  />
                )}
          ...
          {cities.map(city => {
            ...
            return (
              <div
                key={city.id}
                onClick={() => setSelectedCityId(city.id)}
                className={`p-3 rounded-xl border transition-all cursor-pointer flex flex-col gap-2 ${isSelected
